...
const getPaisModel = require('../../services/global/models/pais.model');
const getDepartamentoModel = require('../../services/global/models/departments.model');
const getCiudadModel = require('../../services/global/models/cities.model');
const { globalConnection } = require('../database/connection');

const enviarFacturaConexus = async (transaccionPopulated, branchDb) => {
    try {
        if (!branchDb) throw new Error('Contexto de sucursal es requerido para conexus');
        const Empresa = getEmpresaModel(branchDb);
        const Pais = getPaisModel(globalConnection);
        const Departamento = getDepartamentoModel(globalConnection);
        const Ciudad = getCiudadModel(globalConnection);
...
=========================================================================*/
const enviarNotaCreditoConexus = async (transaccionDevolucion, transaccionOriginal, branchDb) => {
    try {
        if (!branchDb) throw new Error('Contexto de sucursal es requerido para conexus');
        const Empresa = getEmpresaModel(branchDb);
        const Pais = getPaisModel(globalConnection);
